...
export function FlowerSprite({
  ...
  // Same rule the journal/collection use (see program/accounts isHybrid): a bred flower is a
  // hybrid whether its genome is still sealed or has been revealed.
  const isHybrid =
    flower.visualSpeciesId === 255 || flower.genomeStatus === GenomeStatus.Encrypted;

  // Revealed hybrid: the sealed genome has unsealed at least one trait slot, so we render the
  // deterministic 625-combination artwork from the on-chain revealed_trait_mask (5 petal shapes
  // × 5 palettes × 5 cores × 5 stems). The SVG string is produced by the shared hybrid visual
  // system and injected on a px-sized div, matching the other sprite footprints. Any hybrid with
  // a non-zero mask qualifies, Encrypted or Revealed — both read the same four bytes.
  if (isHybrid && flower.revealedTraitMask !== 0) {
  ...
  // Sealed hybrid (revealed_trait_mask === 0): nothing has been unsealed yet, so there is no
  // artwork to derive. Show the generic "unknown blend" bloom until a trait reveals.
  //
